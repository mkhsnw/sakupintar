...
class GoalsPage extends StatelessWidget {
          ...
          child: RefreshIndicator(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  _buildHeader().animate().fadeIn(duration: 350.ms).slideY(begin: 0.08),
                  const SizedBox(height: AppDimensions.lg),
                  _buildGoalEstimation(context).animate(delay: 100.ms).fadeIn().slideY(begin: 0.08),
                  const SizedBox(height: AppDimensions.xl),
                  _buildActiveDream(context, state).animate(delay: 200.ms).fadeIn().slideY(begin: 0.08),
                  const SizedBox(height: AppDimensions.xl),
                  _buildGoalList(context, state).animate(delay: 300.ms).fadeIn().slideY(begin: 0.08),
                  const SizedBox(height: AppDimensions.lg),
                  _buildAddGoalButton(context).animate(delay: 400.ms).fadeIn().slideY(begin: 0.08),
                ],
              ),
            ),
          ),
  ...
  Widget _buildGoalEstimation(BuildContext context) {
    ...
    return FutureBuilder<Map<String, dynamic>>(
      ...
      builder: (context, snapshot) {
        ...
        final estimation = data['insight']?['goal_estimation'] ?? 'Yuk rajin nabung biar cepet capai target!';
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  const Icon(Icons.auto_graph_rounded, color: AppColors.surface, size: 24),
  ...
  Widget _buildActiveDream(BuildContext context, GoalState state) {
    ...
    if (activeGoal == null) {
      return Container(
        ...
        child: Column(
          children: [
            const Icon(Icons.star_border_rounded, size: 48, color: AppColors.neutral),
            const SizedBox(height: AppDimensions.md),
            Text(
    ...
    final daysLeft = activeGoal.deadline.toDate().difference(DateTime.now()).inDays;
    final remaining = activeGoal.targetAmount - activeGoal.savedAmount;
    final dailySave = (daysLeft > 0 && remaining > 0) ? (remaining / daysLeft) : 0.0;
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Text(
                'DEADLINE: ${DateFormat('MMM yy').format(activeGoal.deadline.toDate()).toUpperCase()}',
                style: AppTypography.labelSmall.copyWith(
                  fontWeight: FontWeight.w700,
                  color: AppColors.textSecondary,
                ),
              ...
              Text(
                '${Formatters.formatCurrency(remaining > 0 ? remaining : 0)} TO GO',
                style: AppTypography.labelSmall.copyWith(
                  fontWeight: FontWeight.w700,
                  color: AppColors.textSecondary,
                ),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: () {
                context.push('/transaction/add?type=expense&goalId=${activeGoal.id}');
              },
  ...
  Widget _buildGoalList(BuildContext context, GoalState state) {
    if (state.inactiveGoals.isEmpty) return const SizedBox();

    return Column(
      children: state.inactiveGoals.map((goal) {
        ...
        return Padding(
          ...
          child: _buildGoalCard(
            ...
            statusText: isCompleted ? 'Completed' : 'Inactive',
